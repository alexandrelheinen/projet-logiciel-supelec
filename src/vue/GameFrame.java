...
public class GameFrame extends JFrame implements Observer {
	...
	public GameFrame(String s, int[] modeles, int[][] circuit, int num) {
		...
		for (k = 0; k < modeles.length; k++) {
			try {
				...
				System.out.println("Sprite #"+Integer.toString(k+1)+" charge");
			} catch (Exception e) {
	...
	public void update(Observable obs, Object obj) {
		...
		// si l'objet qui a ete actualise est une voiture...
		if (nom.contains("Voiture")) {
			try {
				Voiture voiture = (Voiture) obs;					// on recupere l'objet qui a gere l'interruption
				...
				flags[voiture.getIndex()] = true;					// cette voiture a ete actualisee
			} finally {
				g.dispose();
			}
		} else { // c'est le cas oe l'objet actualise est le circuit
			try {
				...
				// le temps de jeu
				flags[flags.length - 1] = true;							// ce circuit a ete actualise (meme chose que pour les voitures)
			} finally {
				g.dispose();
			}
		}

		if (testArray(flags)) {						// si tous les elements graphiques importants on ete imprimes
			bf.show();
